# CODA Eesti 🇪🇪

Modern, accessible website for Co-Dependents Anonymous (CODA) Estonia support group.

[![Astro](https://img.shields.io/badge/Astro-FF5D01?style=flat&logo=astro&logoColor=white)](https://astro.build)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind-38B2AC?style=flat&logo=tailwind-css&logoColor=white)](https://tailwindcss.com)
[![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![Cloudflare Pages](https://img.shields.io/badge/Cloudflare_Pages-F38020?style=flat&logo=cloudflare&logoColor=white)](https://pages.cloudflare.com/)

---

## ✨ Features

- 🎨 **CODA.org inspired design** - Clean, calm, and welcoming
- 📱 **Mobile-first** - Optimized for mobile devices
- ♿ **Accessible** - Lighthouse accessibility score 90+
- ⚡ **Fast** - Static site with zero JS by default
- 🔒 **Git-based CMS** - Easy content management for non-technical users
- 🌍 **Multilingual ready** - Estonian primary, international links

---

## 🛠️ Tech Stack

- **[Astro](https://astro.build)** - Modern static site generator
- **[Sveltia CMS](https://github.com/sveltia/sveltia-cms)** - Git-based headless CMS
- **[Tailwind CSS](https://tailwindcss.com)** - Utility-first CSS framework
- **[Shadcn/ui](https://ui.shadcn.com/)** - Accessible UI components
- **[Cloudflare Pages](https://pages.cloudflare.com/)** - Free hosting

📄 **[Read full tech stack decision →](docs/TECH_STACK_DECISION.md)**

---

## 📋 Content

- Contact information & meeting details
- Recovery materials (promises, daily texts, stories)
- Estonian-language book recommendations
- Links to international CODA & 12-step organizations

---

## 🚀 Quick Start

```bash
# Install dependencies
npm install

# Start dev server
npm run dev

# Build for production
npm run build

# Preview production build
npm run preview
```

---

## 📚 Documentation

- **[Meeting Protocol (2025-10-29)](docs/PROTOKOLLID/2025-10-29.md)** - Initial planning meeting
- **[Tech Stack Decision](docs/TECH_STACK_DECISION.md)** - Why we chose this stack
- **[Platform Research](CODA_uurimustöö_veebilehed.md)** - Free platform comparison

---

## 🤝 Contributing

This is a community project for CODA Estonia. Multiple administrators can manage content through Sveltia CMS.

**For content editors:** User guide coming soon

**For developers:** See [TECH_STACK_DECISION.md](docs/TECH_STACK_DECISION.md)

---

## 📄 License

Content copyright © CODA Estonia. Code licensed under MIT.

*(Clarifying copyright with coda.org for recovery materials)*

---

## 🔗 Related Links

- [CODA.org](https://coda.org) - International CODA
- [CODA Germany](https://coda-deutschland.de)
- [CODA UK](https://codauk.org)
- [CODA Canada](https://codacanada.ca)

---

**Built with ❤️ for the Estonian recovery community**
